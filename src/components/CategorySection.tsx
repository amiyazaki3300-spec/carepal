...
export function CategorySection({ category, products, stock, officeUnits, isMobile, onTapProduct }: Props) {
      ...
      {isMobile ? (
        <div className="mob-product-list">
          {[...featured, ...others].map((p) => (
            <ProductCard key={p.id} product={p} products={products} stock={stock} officeUnits={officeUnits} isMobile={isMobile} onTap={onTapProduct} />
          ))}
        </div>
      ) : (
        <>
